...
- Каждый элемент из items должен быть указан в correct с правильной зоной.
- Зоны в correct должны быть из списка zones.
- Несколько элементов МОГУТ относиться к одной и той же зоне (классификация/группировка). Это нормально и допустимо.
- Некоторые зоны могут оставаться пустыми, если ни один элемент к ним не относится.
- len(correct) ОБЯЗАН равняться len(items). Каждый элемент имеет ровно одну зону.
...
- Проверь: в drag_and_drop len(correct) == len(items) и все зоны из correct есть в zones.
- КРИТИЧНО: перед выводом JSON пройдись по КАЖДОМУ заданию и ПЕРЕСЧИТАЙ правильный ответ с нуля. Сравни с тем, что ты поставил в "correct". Если ответ не совпадает — исправь. Примеры типичных ошибок:
  * Неправильный подсчёт слогов (слоги = количество гласных букв в слове: А, О, У, Ы, Э, Я, Е, Ё, Ю, И)
  * Неправильный результат арифметики (сложи/вычти/умножь/раздели заново)
  * Неправильная классификация (перепроверь каждый элемент по правилу из шага 1)
  * correct в quiz не совпадает дословно с текстом в options
